...
# Define temperature API URL and save interval in seconds
...
def getTemp():
    ...
    try:
        ...
        for ind in range(len(data['list'])):
            dayTime = datetime.strptime(data['list'][ind]['dt_txt'], '%Y-%m-%d %H:%M:%S')
            Temp[(dayTime + timedelta(hours=-1)).strftime('%Y-%m-%d %H')] = data['list'][ind]['main']['temp']
            Temp[dayTime.strftime('%Y-%m-%d %H')] = data['list'][ind]['main']['temp']
            Temp[(dayTime + timedelta(hours=1)).strftime('%Y-%m-%d %H')] = float(data['list'][ind]['main']['temp'])
        avg = {}
        for i in Temp:
            key = i.split(' ')[1]
            if key in avg:
...
def hourToDayData(From, data):
    ...
    for d in data:
        if date.strftime('%Y-%m-%d') == d['dateTime'].strftime('%Y-%m-%d'):
            ...
            output.append({"date": date.strftime('%Y-%m-%d'), "yhat": round(value, 2)})
...
def getData(From, To):
    ...
    if From.replace(minute=0) == To.replace(minute=0):
        if (To - From).seconds // 60 != 0:
            ...
            erg['yhat'] = round(erg['yhat'] * ((To - From).seconds // 60) / 60, 2)
            data.append(erg)
    else:
        dayTime = From
        ...
        erg['yhat'] = round(erg['yhat'] * ((newDate - dayTime).seconds // 60) / 60, 2)
        data.append(erg)
        dayTime = newDate
        ...
        if (To - dayTime).seconds // 60 != 0:
            ...
            erg['yhat'] = round(erg['yhat'] * ((To - dayTime).seconds // 60) / 60, 2)
            data.append(erg)
    status = saveUserData(data)
    savePredictData(ldata)
    
    # Modified to return the data for output
    return data  # Change: return data for output
...
def predict():
    ...
    print(json.dumps({"status": "completed", "data": data}))  # Change: Output JSON format

if __name__ == "__main__":
    with open('C:/Users/A/Desktop/Energy-Demand-Forecasting-master/src/webapp/backend/models/model.pkl', 'rb') as f:
        model = pickle.load(f)
        predict()
    sys.stdout.flush()
